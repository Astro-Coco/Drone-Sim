...
class Imu:
    ...
    def compute_noise(self, roll: int, pitch: int, yaw: int) -> tuple[int, int, int]:
        roll_noise = 0
        pitch_noise = 0
        yaw_noise = 0
...
class simulation:
    def __init__(self, dt: float, time: float, mode: str) -> None:
        self.dt = dt
        self.time = 0
        self.timer = time
        self.mode = mode
...
class Drone_software:
    def __init__(self) -> None:
        # rate pid values
        self.PRateRoll, self.PRatePitch, self.PRateYaw = 1.0, 1.0, 0.0
        self.IRateRoll, self.IRatePitch, self.IRateYaw = 0.0, 0.0, 0.0
        self.DRateRoll, self.DRatePitch, self.DRateYaw = 0.0, 0.0, 0.0

        # angle pid values
        self.PRoll, self.PPitch = 2.0, 2.0
        self.IRoll, self.IPitch = 0.0, 0.0
        self.DRoll, self.DPitch = 0.0, 0.0
...
def normalize_quaternion(q: ArrayLike) -> ArrayLike:
...
def update_orientation(
    q: ArrayLike, omega: ArrayLike, dt: float
...
def rotate_vector(v: ArrayLike, q: ArrayLike) -> ArrayLike:
...
def quaternion_to_euler(q: ArrayLike) -> tuple[float, float]:
    w, x, y, z = q
